...
namespace MediaServicesV2.Services.Encoding.Presets
{
    /// <summary>
    ...
    public class MediaServicesPreset : IMediaServicesPreset
    {
        ...
        {
            _ = presetName ?? throw new ArgumentException($@"{nameof(presetName)} is invalid", nameof(presetName));
            if (presetDictionary.TryGetValue(presetName, out string presetValue))
            {
                var replaceWith = timeCodeTimeSpan?.ToString() ?? "{Best}";
                presetValue = presetValue.Replace(ThumbnailStartToken, replaceWith, StringComparison.InvariantCulture);
                return presetValue;
            }
            return presetName;
        }
